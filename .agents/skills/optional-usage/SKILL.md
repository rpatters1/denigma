...
- Use `std::optional<Enum>` when disengagement means that a lookup or mapping produced no value, rather than representing a state in the domain modeled by the enum.
- Do not introduce a duplicate or wrapper enum solely to add an `Unspecified` value to another enum returned by a lookup. Return `std::optional` of the original enum instead.

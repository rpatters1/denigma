...
A `<symbol>` does not name the source font, and that follows from the same reasoning. A symbol identifies a glyph rather than a character, and it is written only because Denigma declined to depend on the source font being installed; naming that font would ask the reader to resolve a SMuFL glyph name out of it, which a legacy font cannot do at all. Size and style are carried over only for a SMuFL source, where every font sets one em to four staff spaces so a point size carries its meaning across, keeping a tempo glyph deliberately smaller than staff size. A legacy font's point size describes only its own design and would mis-scale the substituted glyph, and its bold or italic would only ask for a synthesized slant on a glyph that has none, so both are dropped. Style and weight are nonetheless always stated, since `mx::api::FontData` leaves them unspecified by default and an unspecified style inherits from whatever ran before; a legacy source gets an explicit normal rather than nothing, exactly as ordinary words do.
